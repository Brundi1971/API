...
HWND                                hwnd, hDiscBig, hDiscMid, hDiscLit, hBtn, hBase, hBat1, hBat2, hBat3, hText, hText1, hCeas;
...
bool                                bTriggerRezolva, bShowTheGame;
size_t                              saved_item, intSecventaGame;
sRecordedMovement                   RecordedMovement;
vector<sRecordedMovement>           bufferRezolvare;
...
void CALLBACK catchInt(HWND hwnd, UINT uMsg, UINT timerId, DWORD dwTime);
void redraw_the_game(void);
void rezolva_hanoi (int n, int Stick1, int Stick2, int Stick3);
void show_the_game(void);
                     ...
                     int nCmdShow)
{
    ...
    SetHook();
    //bTriggerCeas = false;
    SetTimer(NULL, 0, 100*NO_SECUNDE,(TIMERPROC) &catchInt);
...
void CALLBACK catchInt(HWND hwnd, UINT uMsg, UINT timerId, DWORD dwTime){
    ...
    //SendMessage(hCeas, WM_SETTEXT, 0, (LPARAM)buffer);
...
void AddControls(HWND hwnd){
    ...
    //hCeas = CreateWindow(TEXT("STATIC"), TEXT("Ceas"), WS_VISIBLE | WS_CHILD, MainWinWidth-CEAS_W, 0, CEAS_W, CEAS_H, hwnd, NULL, NULL, NULL);
    //SendMessage(hCeas, WM_SETTEXT, 0, (LPARAM)"");
    ...
    strCurrentPath.resize(40);
    GetCurrentDirectory(40,&strCurrentPath[0]);

    //cout << strCurrentPath.c_str() << endl;
    ...
    //std::string strImgPath="D:\"
    //hBitmap = (HBITMAP)::LoadImage( NULL, strImgPath.c_str(), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE );

    AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hMenu, "Optiuni");
    AppendMenu(hMenubar, MF_STRING, ITEM_RESET, "Reset");
    AppendMenu(hMenubar, MF_STRING, ITEM_REZOLVA, "Rezolva");
    AppendMenu(hMenubar, MF_STRING, ITEM_STOP, "Stop");
    ...
    AppendMenu(hMenu, MF_STRING, 9, "9 Discuri");
    AppendMenu(hMenu, MF_STRING, 10, "10 Discuri");
...
void rezolva_hanoi (int n, int Stick1, int Stick2, int Stick3){
    ...
    if (n==1) {
        //cout<<n<<"  "<<bat1<<bat2<<endl;
        RecordedMovement.intFromStick=Stick1;
        RecordedMovement.intToStick=Stick2;
        bufferRezolvare.push_back(RecordedMovement);
    }else{
        rezolva_hanoi(n-1,Stick1,Stick3,Stick2);
        //cout<<n<<"  "<<bat1<<bat2<<endl;
        RecordedMovement.intFromStick=Stick1;
        RecordedMovement.intToStick=Stick2;
        bufferRezolvare.push_back(RecordedMovement);
        rezolva_hanoi(n-1,Stick3,Stick2,Stick1);
    }
}

void show_the_game(void){

    size_t  len, intFromStick,intToStick,m,n;
    std::ostringstream                  bufferMsg;

    //cout << intSecventaGame << endl;
    len=bufferRezolvare.size();
    if (intSecventaGame == len){
        //sprintf(buffer,"%d",len);
        bufferMsg.clear();
        bufferMsg << " Numarul de mutari: " << len;
        //cout << buffer;
        //SendMessage(hCeas, WM_SETTEXT, 0, (LPARAM)&bufferMsg.str()[0]);
        bShowTheGame = false;
        MessageBox(hwnd,(LPSTR)&bufferMsg.str()[0], "Mesaj de Informare !", MB_OK | MB_ICONINFORMATION);
        return;
    }

    intFromStick=bufferRezolvare[intSecventaGame].intFromStick;
    intToStick=bufferRezolvare[intSecventaGame].intToStick;

    for (m=0;m<no_discuri;m++){if (M[intFromStick][m] == 0)break;}
    for (n=0;n<no_discuri;n++){if (M[intToStick][n] == 0)break;}

    M[intToStick][n]=M[intFromStick][m-1];
    M[intFromStick][m-1]=0;
    redraw_the_game();
...
void rebuild_the_game(HWND hwnd, int item){
    ...
    if (item==ITEM_RESET) {item=saved_item;}
    if (item==ITEM_REZOLVA) {bufferRezolvare.clear(); rezolva_hanoi(no_discuri,0,1,2); intSecventaGame=0; bShowTheGame=true; return;}
    if (item==ITEM_STOP) {bShowTheGame=false; return;}
